...
def v118_211(ns, t0):
    rh = vol(' \
        |118 *2 pp 2/4 _p 2/4 pp * pp 3/2 mp 1/2 pp \
        |126 *2 pp 2/4 _p 2/4 pp * pp 2/2 mp 2/2 mp \
        |134 pp 4/2 f 4/2 pp \
    ...
    rha = accents(' \
        |118 24/2 \
        |142 *2 *3 1/8 mf 3/8 * 2/4 *3 1/8 f 3/8 * 2/4 * \
        |158 54/2 \
    ', 2/4)
    lh = vol(' \
        |118 p 7/2 p 1/2 pp \
...
def main():
    ...
    if do_50:
        ...
        ns.insert_pedal(PedalUse(t_50+42*2/4, 6*2/4))
    if do_118:
        t_118 = ns.cur_time
